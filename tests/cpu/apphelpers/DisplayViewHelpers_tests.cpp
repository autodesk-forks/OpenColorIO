...
OCIO_ADD_TEST(DisplayViewHelpers, active_display_view)
{
    ...
    OCIO_REQUIRE_NO_THROW(cfg = OCIO::Config::CreateFromStream(is)->createEditableCopy());
    OCIO_CHECK_NO_THROW(cfg->validate());
...
OCIO_ADD_TEST(DisplayViewHelpers, remove_display_view)
{
    ...
    OCIO_REQUIRE_NO_THROW(config = OCIO::Config::CreateFromStream(iss)->createEditableCopy());
